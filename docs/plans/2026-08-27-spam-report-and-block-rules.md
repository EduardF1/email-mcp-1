...
- **Not verified from this sandbox**: the raw-mode masked-password path (typing a password shows `*` per character) only activates on a real TTY, which the sandbox running these checks doesn't have. It needs a human at a real terminal to confirm the masking itself renders correctly; the non-TTY fallback (plain visible input) is what the piped tests exercised.
